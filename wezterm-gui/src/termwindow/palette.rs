...
impl CommandPalette {
    pub fn new(term_window: &mut TermWindow) -> Self {
        ...
        let mut commands = build_commands(GuiWin::new(term_window), mux_pane, filter_copy_mode);

        let pn = term_window.get_active_pane_or_overlay();
        match pn {
            Some(pn_value) => {
                let matcher = SkimMatcherV2::default();
                let (_first_row, lines) = pn_value.get_lines(0..10);
                for line in lines {
                    match matcher.fuzzy_match(line.as_str().as_ref(), "Mic")
                    {
                        Some(score) => {
...
impl Modal for EricWindow{
    ...
    fn computed_element(&self, term_window: &mut TermWindow) -> anyhow::Result<Ref<[ComputedElement]>> {
        ...
        let prompt_element = Element::new(&font, ElementContent::Children(prompt_elements))
            .colors(ElementColors {
                border: BorderColor::default(),
                bg: cloned_pane.pane.palette().background.to_linear().into(),
                text: term_window
                    .config
                    .command_palette_fg_color
                    .to_linear()
        ...
        *self.max_rows_on_screen.borrow_mut() = max_rows_on_screen;
        ...
        for (display_idx, mut c) in ms.iter_mut().take(max_rows_on_screen).enumerate() {
        ...
        let results_element = Element::new(&font, ElementContent::Children(result_elements))
            .colors(ElementColors {
                border: BorderColor::new(
                    term_window
                        .config
            ...
            .min_width(Some(Dimension::Pixels(panel_width )))
            .min_height(Some(Dimension::Pixels(max_rows_on_screen as f32 * metrics.cell_size.height as f32)));

        let preview_border_element = Element::new(&font, ElementContent::Children(vec![]))
            .colors(ElementColors {
                border: BorderColor::new(
                    term_window
                        .config
            ...
            .min_width(Some(Dimension::Pixels(panel_width)))
            .min_height(Some(Dimension::Pixels(half_height)));
        ...
        let element = Element::new(&font, ElementContent::Children(combined))
            .colors(ElementColors {
                border: BorderColor::new(
                    cloned_pane.pane.palette().background.to_linear().into(),
                ),
                bg: cloned_pane.pane.palette().background.to_linear().into(),
                text: term_window
                    .config
            ...
            .min_width(Some(Dimension::Pixels(desired_pixel_width)))
            .min_height(Some(Dimension::Pixels(full_height)));
